...
`aeris.review.json`／Implementation `config/review_gate.v1.json` 是治理記錄；目前沒有 runtime 強制器。README 或 JSON 的 HOLD 不等於舊 shell 程式會拒絕執行。舊命令目前不得啟動，執行式 admission gate 列為未施工的阻擋項。
